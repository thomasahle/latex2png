<script>
  ...
  function showExample(e) {
    e.preventDefault();
    currentExampleIndex = (currentExampleIndex + 1) % latexExamples.length;
    latexContent.set(latexExamples[currentExampleIndex].trim());
  }
</script>

<h1>Convert LaTeX math equations to PNG/JPEG/SVG images</h1>
<p class="description">
  <span class="extra">
