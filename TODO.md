...
- [x] There's a bug where the latex-toolbar menu might be hidden, but it blocks the menu items / other clickable dom elements (e.g. open and close the toolbar, enter fullscreen, open the share menu: "Share Image" can't be clicked and the editor doesn't get a text cursor). Fixed by keeping the force-mounted content `hidden` and `inert` while closed.
