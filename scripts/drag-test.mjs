    ...
    if (deviceScaleFactor === 2) {
      ...
      for (const theme of ['light', 'dark']) {
        ...
        for (const background of ['solid', 'transparent', 'custom']) {
          ...
          if (background === 'custom') assert.equal(color, 'rgb(32, 96, 192)', 'cursor preview uses the custom color');
          ...
          if (background === 'custom') assert.deepEqual(pixel, [32, 96, 192, 255], 'dragged PNG uses the custom color');
          assert.equal(pixel[3], background === 'transparent' ? 0 : 255, `${theme}: PNG background follows the dropdown`);
      ...
      await cdp.detach();
      console.log('Drag background: transparent/solid/custom cursor previews and PNGs in light/dark themes');
    }
    await context.close();
  }
} finally {
